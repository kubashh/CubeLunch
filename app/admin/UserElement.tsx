...
export default function UserElement({ name, email, id, role }: User) {
  return (
    <form className="mb-1 border-t-3 border-zinc-900 p-2 grid grid-cols-4">
      <span>{name}</span>
      <span>{email}</span>
      <select
        defaultValue={role}
        ...
        onChange={async ({ target }) => {
          await changeUserRole(id, Number(target.value))
        }}
      >
